# SQL & Database Fundamentals — Day 1 to Day 11

> **Compilation note:** Day 6–Day 11 content below is compiled from the SQL study chat. The exact full Day 1–Day 5 chat messages are not available in the current retrievable conversation context, so those days are reconstructed from the previously established SQL/Database Fundamentals syllabus and progress topics. No Day 6–Day 11 study topic is intentionally omitted.

---

# 🗓️ DAY 1 — DATABASE FUNDAMENTALS

## 🎯 Day 1 Goal

Understand the fundamentals of databases before writing SQL.
...
It is software used to create, store, manage, retrieve, update, and control data in databases.
...
| May not use relationships between tables | Uses relationships between tables |
| Can support different storage models | Uses tables/relations |
| Relationship enforcement is not the defining feature | Primary/Foreign Keys are central |
| Example: some file-based/simple systems | PostgreSQL, MySQL, Oracle |
...
| Structured tables | Flexible/document/key-value/graph/column models |
| Uses SQL | Uses database-specific APIs/query languages |
| Strong schema is common | Flexible schema is common |
| Excellent for relational data | Useful for flexible/high-scale workloads |
| Examples: PostgreSQL, MySQL | MongoDB, Redis, Cassandra |

## 8. Basic Database Terms
...
- Install/use PostgreSQL or MySQL.
- Open the SQL client.
- Create a database.
- Create a simple table.
- Insert records.
- Read records.
- Understand the table/row/column structure.

---

# 🗓️ DAY 2 — DATABASE + SQL BASICS

## 🎯 Day 2 Goal

Understand how SQL statements work and practice basic database operations.
...
The exact available types vary by database system.

## 4. INSERT

```sql
INSERT INTO employees
VALUES (1, 'Rahul', 25, 40000);
```
...
Used to modify a database object such as a table.
...
Be careful: dropping a table removes the table and its data.
...
Used for permissions/access control.
...
Create a database and an employees table. Insert several records and practice:
...
# 🗓️ DAY 3 — THREE-SCHEMA ARCHITECTURE + ER DIAGRAMS + KEYS + RELATIONSHIPS

## 🎯 Day 3 Goal

Understand how databases are structured internally and how tables are related.
...
The user/application view of the data.
...
It deals with storage structures, files, indexes, and physical organization.

## 2. Data Independence

### Logical Data Independence

Changes to the conceptual schema should not require changes to external views/applications.

### Physical Data Independence

Changes to physical storage should not require changes to the logical schema.
...
Usually this requires a junction/bridge table:
...
A Foreign Key references a key in another table and helps enforce a relationship.
...
# 🗓️ DAY 4 — TRANSACTIONS + ACID + CAP THEOREM

## 🎯 Day 4 Goal

Understand how databases handle operations reliably and how distributed databases deal with consistency, availability, and partitions.
...
If adding to B fails, the removal from A should also be undone.

## 4. Consistency

A transaction takes the database from one valid state to another valid state while respecting defined rules.
...
In a distributed system experiencing a network partition, the system cannot simultaneously guarantee both strong consistency and availability.

### Consistency

Every appropriate read sees the expected latest value.
...
CAP is specifically about distributed systems and network partitions. It is not simply:

```text
Choose any two at any time.
```

The key situation is what happens **when a partition occurs**.

## 🧪 Day 4 Practical

Use transaction examples such as:
...
- What should happen if operation 1 succeeds?
- What if operation 2 fails?
- Where would rollback happen?
- What data must remain consistent?

---

# 🗓️ DAY 5 — NORMALIZATION

## 🎯 Day 5 Goal

Understand why databases are split into related tables and how normalization reduces data problems.
...
The same information is unnecessarily repeated.
...
If the instructor changes, multiple rows may need to be updated.

## 5. Insert Anomaly

You may be unable to store information about one entity without unrelated information.

## 6. Delete Anomaly

Deleting one record may accidentally remove the only copy of useful information.

## 7. First Normal Form — 1NF

A table should contain atomic values and should not have repeating groups.
...
or separate phone rows depending on the model.

## 8. Second Normal Form — 2NF

A table should be in 1NF and non-key attributes should depend on the whole primary key, not just part of a composite key.
...
Then use keys and relationships.

## 11. Denormalization

Denormalization intentionally introduces some redundancy to improve certain read/query patterns.

For this fundamentals week, understand the concept only.

## 🧪 Day 5 Practical

Take an unnormalized table containing:
...
Then split it into related tables and identify PK/FK relationships.
...
# 📚 SQL & Database Fundamentals Progress
...
This study track is focused on **Database + SQL Fundamentals**, not Advanced SQL.
...
## Final Fundamental Mental Model
